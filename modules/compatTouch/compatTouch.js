...
function init() {
    MODULES = compatTouch.MODULES;
}
compatTouch.init = init;
...
function clickOrSwipeRoot(x1, y1, x2, y2, duration) {
    ...
    if (MODULES.shellCmd == null) {
        throw new Error("shellCmd module not loaded");
    } else {
        MODULES.shellCmd.privShell(cmd, logString);
    }
}
...
compatTouch.swipe = compatSwipe;

function compatPress(x, y, duration) {
    log("使用滑动模拟按住");
    compatSwipe(x, y, x, y, duration);
}
compatTouch.press = compatPress;

module.exports = compatTouch;