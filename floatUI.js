...
floatUI.scripts = [
    ...
    },
    {
        ...
        introduction: "只能在进入战斗后启动，启动后会自动选盘、打完本局战斗，然后结束。“镜层周回”脚本在停用了识图自动战斗的情况下，每一局战斗调用的就是这个脚本。\n"
                     +"虽然最常用于镜层，但在剧情副本战斗中也可以启用，可以应对没有初回AUTO（现在很少碰到了）的情况。强烈推荐只在我方战力保证能够碾压剧情副本敌人时使用这个脚本！\n"
                     +"考虑到点击行动盘在某些情况下会偶尔点不到，所以，在点击前3个盘时可能会故意暂时跳过某些盘不点，从而抵消掉偶尔点盘没反应的问题，避免在点击行动盘时出现死循环（“镜层周回2(备用,无脑123盘)”脚本没有加入这个逻辑）。",
    },
    {
        name: "录制闪退重开选关动作",
        fn: tasks.recordSteps,
        introduction: "正如其名，这个脚本用来录制一连串的点击/拖动/检测文字是否存在等动作，用来在闪退/掉线自动重开的时候自动重新选关。\n"
    ...
    },
    {
        ...
        introduction: "正如其名，这个脚本用来重放之前录制（或导入）的一连串的点击/拖动/检测文字是否存在等动作，用来在闪退/掉线自动重开的时候自动重新选关。\n"
                     +"重放每一步操作时都会进行强制的“安全检查”，尽最大可能避免在抽卡/记忆结晶强化或界限突破等界面产生危险的误操作。\n"
                     +"在选关动作录制完成后，强烈推荐多次测试重放选关动作（很显然，每次重放开始前都需要先回到首页），确保一切无误！",
    },
    {
        name: "导入动作录制数据",
